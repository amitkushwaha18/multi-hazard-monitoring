...
const LandingPage = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    // Deterministic PRNG (mulberry32) so the star field is stable across
    // renders and devices - pure decoration, no mock data.
    ...
    for (let i = 0; i < numParticles; i++) {
      ...
      const r = Math.pow(rng(), 1.5) * (Math.max(width, height) * 0.7);
      const armAngle = armIndex * ((2 * Math.PI) / numArms);
      const spiralSpread = r * 0.005;
      const theta = armAngle + spiralSpread + (rng() - 0.5) * 0.45;
